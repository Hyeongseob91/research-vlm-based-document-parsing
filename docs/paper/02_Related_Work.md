...
## 2.5 Causal Evaluation in RAG Pipelines

Most RAG evaluation studies measure end-to-end performance (answer quality, retrieval accuracy) without isolating which pipeline component contributes to improvement. When a VLM-based parser outperforms traditional OCR in a full RAG pipeline, the improvement could stem from better text extraction, better structure preservation, better chunking, or some combination.

Few studies systematically decompose this causal chain. Our two-step evaluation methodology addresses this gap by isolating each link:
1. **Structure → Chunking**: Controlling chunking strategy, varying parser quality
2. **Chunking → Retrieval**: Controlling parser, measuring retrieval impact

### 2.5.1 Label-Free Chunk Quality Metrics

MoC (Metrics of Chunks) proposes label-free metrics for chunk quality evaluation:
- **Boundary Clarity (BC)**: Embedding cosine distance across chunk boundaries — higher distance indicates cleaner topic separation
- **Chunk Stickiness (CS)**: Intra-chunk embedding similarity — higher similarity indicates better coherence

These metrics correlate with downstream retrieval performance (BC↔ROUGE-L: r=0.88) without requiring labeled QA pairs, making them suitable for large-scale comparative evaluation.

## 2.6 Gap Analysis

Despite advances in document parsing:

1. **No causal decomposition**: Studies compare end-to-end but don't isolate structure → chunking → retrieval links
2. **Missing structural analysis**: Few studies quantify structural preservation impact on chunk quality
...
5. **Chunking strategy interaction**: No comparison of how different chunking strategies perform on structured vs unstructured parser output

This study addresses these gaps with a two-step causal evaluation methodology and multi-metric framework spanning parsing, chunking, and retrieval.
